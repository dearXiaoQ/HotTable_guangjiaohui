...
public class MultiSelectSpinner extends android.support.v7.widget.AppCompatSpinner implements OnMultiChoiceClickListener {
    ...
    public void onClick(DialogInterface dialog, int which, boolean isChecked) {
        if (_selection != null && which < _selection.length) {
            ...
            Variety variety = IndControlActivity.varietyList.get(which);
            Log.i("variety", "variety = " + variety.toString());
            if(isChecked) {
                IndControlActivity.countDownList.add(variety);
            } else {
                IndControlActivity.countDownList.remove(variety);
            }
            _proxyAdapter.clear();
